...
- **`common.rs`** — shared types touched by every other module.
  ...
  - Resources: `Gold` (per-`PlayerSlot` pool), `GameState`, `GameMode` (`OneVsOne`/`TwoVsTwo`, selected on the SideSelect screen), `GameSettings`, `SettingsTab`, `SettingsOrigin`, `MatLibrary` (procedural meshes/materials still used by units, torches, ghosts, zone markers), `EnvAssets` (glTF scene handles for the base/tower buildings and the desert props — loaded once by `load_env_assets`), `PlacementMode`, `PlayerControllers`, `PlayerNations` (per-`PlayerSlot` chosen `Nation`, committed at launch), `MenuFocus`, `TimeOfDay`, `GameTime`, `DlssAvailable`, `RaytracingAvailable`, `AtmosphereHandle`.
...
- **`setup.rs`** — `Startup` systems and most static scene authoring. `init_mat_library` populates `MatLibrary` (must run before anything spawns meshes — enforced via `.chain()` in `main.rs`); `load_env_assets` (also `Startup`, chained before `setup_world`) kicks off the async load of the building + desert-prop glTF scenes into `EnvAssets`. `setup_world` spawns the camera (fixed 3/4 view with `Hdr`, atmosphere, bloom, fog), the sun, the desert ground, mountains, sky, zone markers, and the scenery. **Scenery is procedural**: `spawn_scenery` lays a jittered grid of desert-prop `SceneRoot`s (cactus / dead tree / ruins / skull / stone / stone arch, two glTF variants each) across the background from outside the play zone out to the mountains, keeping a central gameplay rectangle clear; each cell's kind is drawn by `DesertProp::weight` (rocks/cacti/trees common, ruins/arches rare) with random yaw and per-instance scale via a tiny deterministic xorshift `Rng`. Every Meshy export is normalized to a ~1.9-unit box centered at origin, so each kind has a `*_SCALE` + `*_LIFT` (ground seat) const in `common.rs`. `spawn_arena` (an `Update` system) builds the bases + rocks lazily on the `Menu→Playing` transition so the layout can reflect the chosen `GameMode` (two or four bases): `spawn_castle` spawns the `adamar_base.glb` model under the `Base` entity (rotation = `side.base_rotation()` × `BASE_MODEL_YAW_OFFSET` so the entrance faces the centre) plus corner torches; `spawn_rock` reuses the `irrhakur_stone` prop as the mining `Rock`. `spawn_torch` is the shared helper (pole + hidden flame + doused `PointLight`) used by bases and towers; the day/night `update_torches` lights them. Also hosts the Solari/DLSS sync systems (gated on cargo features), `update_torches` / `animate_sun`, the archer glTF loader (`load_archer_assets`) + animation-graph builder (`build_archer_graph`), and `debug_camera_control` — a free-fly debug camera (mouse + keyboard, otherwise unused since the game is gamepad-only): hold RMB to look, WASD/Space/LShift to fly, LCtrl to boost, scroll to change speed, **R** to snap back to `CAMERA_DEFAULT_POS`. Runs in every `GameState`.

- **`units.rs`** — unit lifecycle, AI and rendering rigs.
  - Three unit kinds spawned by `spawn_soldier`, `spawn_miner`, `spawn_archer`. The soldier and miner share a procedural body+head+limbs rig (`UnitRig`) so `animate_units` can drive walking/attack/hurt/death animations from a single component (`UnitAnim`). The **archer is a rigged glTF model** (Meshy export, `assets/models/adamar/characters/adamar_archer_biped_Animation_*_withSkin.glb`) instead: `spawn_unit` branches early for `UnitKind::Archer`, spawning a `SceneRoot` child (scale + facing correction via `ARCHER_MODEL_SCALE`/`ARCHER_MODEL_YAW_OFFSET`) and tagging the root `ArcherModel` with no `UnitRig`. It carries the same `UnitAnim` flags (set by `combat_tick`/`process_damage_effects`), but is animated by `animate_archer`, not `animate_units` (which requires `UnitRig`). Meshy scrambles the internal animation names, so clips are loaded **one file per action** (the file path is the source of truth, not the internal name): `setup.rs::load_archer_assets` loads the scene/mesh from the Walking file and one `AnimationClip` per file (walk, `Archery_Shot`, two hurt reactions `Face_Punch`/`Slap`, `Shot_in_the_Back_and_Fall`); the clips retarget onto the scene's skeleton since every file shares the same rig. `setup.rs::build_archer_graph` waits for those clips to decode, then builds the `AnimationGraph` and caches node indices + playback speeds. Paths live in the `ARCHER_*_PATH` consts in `common.rs`. The archer faces its target by smoothly rotating the whole entity toward `UnitAnim.face_yaw` (no dedicated turn clip — in-place Mixamo turn clips bake a fixed rotation that fights the entity yaw). Hurt reactions hard-cut in/out so they never blend over the shot pose. `bind_archer_animation_player` attaches the graph + `AnimationTransitions` to the scene's async-instanced `AnimationPlayer` and links it back via `ArcherAnimState`; `bind_archer_bow_hand` resolves the skeleton's `LeftHand` bone (`ARCHER_BOW_HAND_BONE`) the same way (walk up from the named bone to the archer root) and records it on `ArcherAnimState.left_hand`. `combat_tick` writes the archer's desired facing to `UnitAnim.face_yaw` and moves it with `move_toward_xz` (no rotation); `animate_archer` owns the rotation — it smoothly pivots toward `face_yaw` and runs the clip state machine: death > fresh hit (only while stationary, so a hit never breaks the walk) > attack > walk > idle. The `Archery_Shot` clip releases to the model's **left**, not straight ahead, so when shooting `combat_tick` sets `face_yaw = target_angle + ARCHER_SHOT_YAW_OFFSET` (−90°) to turn the archer's left toward the target, making the release read as aimed; otherwise it faces the advance direction. **Arrow release is driven by the animation, not a timer:** while aimed, `combat_tick` keeps `ArcherAnimState.pending_shot` set to a snapshot of the target (entity/pos/damage), clearing it when not aimed or with no target; `animate_archer` loops the shot clip and fires one arrow each time the clip's `completions()` increments (the end of a draw-release cycle), spawning it from the `LeftHand` bone's world position (falling back to `ARCHER_HAND_OFFSET` in the entity's local frame only until the bone resolves). The first arrow therefore waits out the turn and one full clip cycle; the kite/advance logic keeps running meanwhile. It alternates the two hurt clips on each hit and owns the archer's `hurt_t`/`death_t` bookkeeping; `cleanup_dead_units` despawns archers after the longer `ARCHER_DEATH_DURATION` so the fall clip completes.
